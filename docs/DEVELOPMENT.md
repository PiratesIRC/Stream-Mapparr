...
- **Leading box-bar bouquet-tag strip in `normalize_name`.** `_LEADING_BAR_TAG_RE` removes
  a leading tag enclosed in the box-drawing bars U+2503 or U+2502 (a bouquet name in those
  bars before `NPO 1` becomes `NPO 1`). The ASCII pipe is not a delimiter here.
- **Zero-width / invisible format-char strip in `normalize_name` (bug-105, issue #36).**
  ...
  invisible padding some providers wrap around a decorative block glyph (a U+258E block
  between the country prefix and the name). The visible glyph (category `So`) was already
  handled by the `_DECORATOR_CATS` pass,
  but the surrounding `Cf` chars are matched by neither `\s` nor `_DECORATOR_CATS`, so they
  survived the whole pipeline (the name kept invisible characters around `BBC 1`) and
  silently tanked the provider's match rate.
...
- **Box-bar delimiters in `GEOGRAPHIC_PATTERNS`.** The box-drawing bars U+2503 and U+2502
  are accepted as colon-equivalents after a 2-3 letter code, and as a matched pair around
  the code (the same bar on both sides only). Stream-Mapparr has no `PROVIDER_PREFIX_PATTERNS` list, so only
...
- **Version scheme:** calver `1.MAJOR.DDDHHMM` (UTC day-of-year + HHMM). Always
  bump with `scripts/bump_version.py` so `plugin.py` and `plugin.json`
  stay in lockstep.
...
- **Validate the zip before attaching it:** `python scripts/validate_zip.py
  Stream-Mapparr.zip` must print OK (see the packaging note in section 5 tooling - bug-087).
- **`validate_zip.py` refuses CRLF in any text entry since 2026-09-06**, alongside the
  bug-087 separator check. Line endings are also fixed at the source (bug-118):
  `.gitattributes` carries `* text=auto eol=lf`, and `build_release_zip.py` forces LF.
...
- **`docker cp` and stdin-piped `docker exec` trip the shell harness here.** Deploy by
  ...
  `docker exec` works).
- **Build the release zip with `python scripts/build_release_zip.py` (`git archive` from the index) - NEVER PowerShell
...
- **Zone routing is per-channel, gated by `_zone_routed_map`** (bug-068). It only
  ...
  change goes to the shared core (see "Shared matcher core" in section 3), not a hand-port - but
...
release.json               # release contract: package dir, tag prefix, changelog, version files
CHANGELOG.md               # consolidated changelog (moved out of the shipped package 2026-09-06)
scripts/
  bump_version.py          # next UTC calver into every release.json version file + changelog stub
  check_version_sync.py    # CI/hook: every version file, the changelog top entry and the tag agree
  validate_zip.py          # release zip guard: separators, package root, dev files, CRLF
  build_release_zip.py     # git archive from the index (LF forced) then validate
  release_config.py        # shared helper for the four above; tools_manifest.json pins them
...
- The ~50 historical `CHANGELOG_v*.md` / `QUICK_SUMMARY_*.md` / `BUG_REPORT_*.md`
  files duplicate the consolidated `CHANGELOG.md`; archive them
